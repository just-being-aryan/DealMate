...
`.trim();
  ...
  const handleExportReport = (result: AnalysisResult) => {
    ...
    a.href = url;
    a.download = `analysis-report-${result.fileName.replace(/\.[^/.]+$/, "")}-${new Date()
      .toISOString()
      .split("T")[0]}.txt`;
    a.click();
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              Upload financial statements, contracts, or other documents for AI analysis
            </p>
                  ...
                  <div className="grid grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-bold">
                        ${(result.financialMetrics.revenue / 1000000).toFixed(1)}M
                      </p>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-lg font-bold">{result.financialMetrics.profitability}%</p>
                    </div>
                  </div>
                )}
      ...
      <div className="bg-card rounded-xl p-6 w-11/12 md:w-2/3 max-h-[90vh] overflow-y-auto shadow-lg relative">
        <h2 className="text-xl font-bold mb-4 text-foreground">
          Analysis Details: {selectedResult.fileName}
        </h2>

        <div className="flex items-center gap-4 mb-4">
          <Badge variant="outline" className={getRiskColor(selectedResult.riskScore)}>
            Risk: {selectedResult.riskScore}%
          </Badge>
          <span className="flex items-center gap-1 text-foreground">
            Sentiment: {selectedResult.sentiment.charAt(0).toUpperCase() + selectedResult.sentiment.slice(1)}
          </span>
        </div>

        {selectedResult.financialMetrics && (
          <div className="grid grid-cols-3 gap-6 mb-4 p-4 bg-muted/30 rounded-lg">
            <div>
              <p className="text-sm text-muted-foreground">Revenue</p>
              <p className="text-lg font-bold text-foreground">
                ${(selectedResult.financialMetrics.revenue / 1000000).toFixed(1)}M
              </p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Growth</p>
              <p className="text-lg font-bold text-success">+{selectedResult.financialMetrics.growth}%</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Profit Margin</p>
              <p className="text-lg font-bold text-foreground">{selectedResult.financialMetrics.profitability}%</p>
            </div>
          </div>
        )}
            ...
            <div className="mb-4">
              ...
              <ul className="list-decimal list-inside space-y-1">
                {selectedResult.keyFindings.map((finding, i) => (
                  <li key={i} className="text-primary">{finding}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="mb-4">
              ...
              <ul className="list-disc list-inside space-y-1">
                {selectedResult.recommendations.map((rec, i) => (
                  <li key={i} className="text-primary">{rec}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="flex justify-end mt-4 gap-2">
              ...
              <Button variant="default" onClick={() => handleExportReport(selectedResult)}>Export Report</Button>
            </div>
          </div>
        </div>
      )}
